...
The Chrome extension bridge provider is opt-in and only activates when selected with `--provider chrome-extension`. Its daemon listens on `127.0.0.1`, validates per-session CDP tokens, and treats `browser.close` as detach so it does not close the user's desktop Chrome.
